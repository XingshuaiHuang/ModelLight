...
class FRAPPlusAgent(Agent):
    ...
    def construct_weights(self, dim_input, dim_output):
        ...
        # for phase, one-hot
        weights['embed_w2'] = tf.Variable(tf.random_uniform_initializer(minval=-0.05, maxval=0.05)([2, 4]),
                                          name='embed_w2')
        ...
        weights['relation_embed_w4'] = tf.Variable(tf.random_uniform_initializer(minval=-0.05, maxval=0.05)([2, 4]),
                                                   name='relation_embed_w4')

        weights['feature_conv_w1'] = tf.Variable(
            tf.glorot_uniform_initializer()([1, 1, 32, self.dic_agent_conf["D_DENSE"]]), name='feature_conv_w1')
        ...
        weights['combine_conv_w1'] = tf.Variable(
            tf.glorot_uniform_initializer()([1, 1, self.dic_agent_conf["D_DENSE"], self.dic_agent_conf["D_DENSE"]]),
    ...
    def construct_forward(self, inp, weights, reuse, norm, is_train, prefix='fc'):
        ...
        lane_conv = tf.nn.conv2d(feature_map, weights['feature_conv_w1'], [1, 1, 1, 1], 'VALID', name='feature_conv') + \
                    weights['feature_conv_b1']
        ...
        # second conv layer
        hidden_layer = tf.nn.conv2d(combine_feature, weights['combine_conv_w1'], [1, 1, 1, 1], 'VALID',
                                    name='combine_conv') + \
        ...
        _shape = (-1, self.num_actions, self.num_actions - 1)  # self.num_actions from Agent class; = "num_phases"
        before_merge = tf.reshape(before_merge, _shape)
        out = tf.reduce_sum(before_merge, axis=2)  # shape: (-1, self.num_actions)
