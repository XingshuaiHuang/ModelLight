...
class BatchEpisodes(object):
    ...
    def append(self, observations, actions, new_observations, rewards, batch_ids):
        ...
        for observation, action, new_observation, reward, batch_id in zip(
            ...
            # print('@@@@@@@@@@@@@@@@@@@new_observation is {}@@@@@@@@@@@@@@@@@@'.format(new_observation))
            # print('^^^^^^^^^^^^^^^^^^^new_observations is {}^^^^^^^^^^^^^^^^^^^'.format(new_observations))
            # print('@@@@@@@@@@@@@@@@@@@reward is {}@@@@@@@@@@@@@@@@@@'.format(reward))
            # print('^^^^^^^^^^^^^^^^^^^rewards is {}^^^^^^^^^^^^^^^^^^^'.format(rewards))

            self.total_samples.append([list(observation), list(action), list(new_observation), list(reward), 0, 0])  # ***transitions
            # empty observation = [{'cur_phase': [0], 'lane_num_vehicle': [0, 0, 0, 0, 0, 0, 0, 0]}]
            # print('----------------total_samples is {}----------------'.format(self.total_samples))
            self.tot_x.append(observation[0]['lane_num_vehicle'] + observation[0]["cur_phase"])  # state [0,0,0,0,0,0,0,0, 0]
    ...
    def forget(self):
        self.total_samples = self.total_samples[-1 * self.dic_agent_conf['MAX_MEMORY_LEN']:]  # keep only the last 2000 samples
