...
class Intersection:
    ...
    def __init__(self, inter_id, dic_traffic_env_conf, eng):
        ...
        self.inter_name = "intersection_{0}_{1}".format(inter_id[0], inter_id[1])  # 2, 2
    ...
    def _update_feature(self):
        ...
        dic_feature["cur_phase"] = phase  # get_state
        ...
        dic_feature["lane_num_vehicle"] = self._get_lane_num_vehicle(self.list_entering_lanes)  # get_state
        ...
        dic_feature["lane_num_vehicle_been_stopped_thres1"] = self._get_lane_num_vehicle_been_stopped(1,
                                                                                                      self.list_entering_lanes)  # get_reward
    ...
    def get_feature(self):
        return self.dic_feature  # def _update_feature()
    ...
    def get_state(self, list_state_features):
        dic_state = {state_feature_name: self.dic_feature[state_feature_name] for state_feature_name in
                     list_state_features}
        # list_state_features: ["cur_phase", "lane_num_vehicle"]
        return dic_state

    def get_reward(self, dic_reward_info):  # "DIC_REWARD_INFO": {"sum_num_vehicle_been_stopped_thres1": -0.25}
        dic_reward = dict()
        ...
        if self.dic_traffic_env_conf['REWARD_NORM']:  # store True
            # normalize the reward
            reward = - 2 * dic_reward["sum_num_vehicle_been_stopped_thres1"] / (
                        40 * len(self.dic_feature["lane_num_vehicle_been_stopped_thres1"])) + 1
        else:
            reward = 0
            for r in dic_reward_info:  # "sum_num_vehicle_been_stopped_thres1"
                if dic_reward_info[r] != 0:  # -0.25
                    reward += dic_reward_info[r] * dic_reward[
                        r]  # -0.25 * np.sum(self.dic_feature["lane_num_vehicle_been_stopped_thres1"])
        return reward


class CityFlowEnv:  # used in replay
    ...
    def __init__(self, path_to_log, path_to_work_directory, dic_traffic_env_conf):
        ...
        """
            ...
            However, it will somehow hang in the multi-process. Therefore, another version of cityflow is used here, 
        ...
        self.list_intersection = None  # defined in reset function, Intersection class
    ...
    def reset(self):
        ...
        self.list_intersection = [Intersection((i + 1, j + 1), self.dic_traffic_env_conf, self.eng) for i in
                                  range(self.dic_traffic_env_conf["NUM_ROW"])  # 1, 1
    ...
    def step(self, action):
        ...
        for i in range(self.dic_traffic_env_conf["MIN_ACTION_TIME"]):
            ...
            if self.dic_traffic_env_conf['DEBUG']:
                ...
                if i == 0 and self.dic_traffic_env_conf["VERBOSE"]:
                    # pass
                    print("time: {0}, phase: {1}, time this phase: {2}, action: {3}".format(instant_time,
                                                                                            before_action_feature[0][
                                                                                                "cur_phase_index"],
    ...
    @staticmethod  # 装饰器：用法与类外函数很像，函数属于类，但是使用的时候不需要访问它所属的类
    ...
    def bulk_log(self):
        ...
        for inter_ind in range(len(self.list_inter_log)):
            path_to_log_file = os.path.join(self.path_to_log, "inter_{0}.pkl".format(inter_ind))
            #  self.path_to_log: os.path.join(dic_path['PATH_TO_WORK_DIRECTORY'],
            # 'test_round', task, 'tasks_round_' + str(batch_id))
            f = open(path_to_log_file, "wb")
