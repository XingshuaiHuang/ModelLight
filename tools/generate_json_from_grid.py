...
def isHorizontal(road):
    return dx[road["direction"]] != 0  # output False or True
...
def gridToRoadnet(rowNumber, columnNumber, rowDistances, columnDistances, outRowDistance, outColumnDistance,
    ...
    def shouldDraw(road):
        return isTruelyInside(road["fromx"], road["fromy"]) or isTruelyInside(road["tox"], road["toy"])
    ...
    for i in range(rowNumber):
        for j in range(columnNumber):
            if j > 0:
                x[i][j] = x[i][j - 1]
                y[i][j] = y[i][j - 1] + columnDistances[j - 1]
            elif i > 0:
                x[i][j] = x[i - 1][j] + rowDistances[i - 1]
                y[i][j] = y[i - 1][j]
            else:
            ...
            for k in range(4):
                ...
                if not isInside(ni, nj):
                    ...
                    road = {
                        ...
                        "startIntersection": "intersection_%d_%d" % (i, j),
                        "endIntersection": "intersection_%d_%d" % (ni, nj)
                    }
                roads[i][j][k] = road
    ...
    for i in range(rowNumber):
        for j in range(columnNumber):
            ...
            inRoads = list(map(lambda k: roads[i - dx[k]][j - dy[k]][k],
                               filter(lambda k: isInside(i - dx[k], j - dy[k]), range(4))))
                    ...
                    try:
                        ...
                        for c in range(len(roada["lanes"])):
                            ...
                            for d in range(len(roadb["lanes"])):
                                # dx, dy = getRoadUnitVector(roada)
                                # width = abs(dx * ())
                                # outTurnPoints = getOutTurnPoints(roada, width, c)
                                # inTurnPoints = getInTurnPoints(roadb, width, d)
                                path = {
                                    "startLaneIndex": c,
                                    "endLaneIndex": d,
        ...
        for j in range(columnNumber):
            for k in range(4):
                if roads[i][j][k] is not None and shouldDraw(roads[i][j][k]):
                    ni, nj = i + dx[k], j + dy[k]
                    # if isEdge(i, j) and isEdge(ni, nj):
                    #     continue
                    roads[i][j][k].pop("direction")
                    roads[i][j][k].pop("fromx")
                    roads[i][j][k].pop("fromy")
                    roads[i][j][k].pop("tox")
                    roads[i][j][k].pop("toy")
